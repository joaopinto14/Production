...
# Function to set the socket for PHP-FPM
set_config_socket_php_fpm() {
    if [ ! -d "/run/php" ]; then
        mkdir -p /var/run/php || { echo "Failed to create directory '/var/run/php'."; exit 1; }
        chown -R nginx:www-data /var/run/php || { echo "Failed to set permissions for '/var/run/php'."; exit 1; }
    fi
}

# Function to set permissions for the project directory
set_permissions_directory_project() {
    chown -R nginx:www-data /var/www/html || { echo "Failed to set permissions for '/var/www/html'."; exit 1; }
}
